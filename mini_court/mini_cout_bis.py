...
class MiniCourt():
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                ...
                if closest_player_id_to_ball == player_id:
                    ...
                    # Store the shot position
                    if player_id == 1:
                        self.player1_shots.append(mini_court_ball_position)
                    else:
                        self.player2_shots.append(mini_court_ball_position)
    ...
    def generate_heatmap(self):
        # Convert shot lists to numpy arrays for seaborn
        player1_shots = np.array(self.player1_shots)
        player2_shots = np.array(self.player2_shots)

        plt.figure(figsize=(10, 8))

        # Plot heatmap for player 1 shots
        sns.kdeplot(
            x=player1_shots[:, 0], y=player1_shots[:, 1],
            cmap="Blues", shade=True, alpha=0.5, label='Player 1'
        )

        # Plot heatmap for player 2 shots
        sns.kdeplot(
            x=player2_shots[:, 0], y=player2_shots[:, 1],
            cmap="Greens", shade=True, alpha=0.5, label='Player 2'
        )

        # Overlay the mini-court
        for i in range(0, len(self.drawing_key_points), 2):
            plt.scatter(self.drawing_key_points[i], self.drawing_key_points[i+1], color='red')
        for line in self.lines:
            plt.plot([self.drawing_key_points[line[0]*2], self.drawing_key_points[line[1]*2]], 
                    [self.drawing_key_points[line[0]*2+1], self.drawing_key_points[line[1]*2+1]], color='black')

        net_start_point = (self.drawing_key_points[0], (self.drawing_key_points[1] + self.drawing_key_points[5]) / 2)
        net_end_point = (self.drawing_key_points[2], (self.drawing_key_points[1] + self.drawing_key_points[5]) / 2)
        plt.plot([net_start_point[0], net_end_point[0]], [net_start_point[1], net_end_point[1]], color='blue')

        plt.legend()
        plt.title('Heatmap of Shot Distribution')
